...
func TestRandomSizedTorrents(t *testing.T) {
	...
	seeder, err := autobind.NewLoopback().Bind(torrent.NewClient(TestingSeedConfig(t, t.TempDir())))
	require.NoError(t, err)
	defer seeder.Close()

	leecher, err := autobind.NewLoopback().Bind(torrent.NewClient(TestingLeechConfig(t, t.TempDir())))
	require.NoError(t, err)
	defer leecher.Close()
...
func TestClientAddressInUse(t *testing.T) {
	...
	cl, err := autobind.NewAutobindSpecified(":50007").Bind(torrent.NewClient(torrent.TestingConfig(t)))
	require.Error(t, err)
	require.Nil(t, cl)
}
...
func TestClientHasDhtServersWhenUtpDisabled(t *testing.T) {
	...
	cc.DisableUTP = true
	cc.NoDHT = false
	cl, err := autobind.NewLoopback().Bind(torrent.NewClient(cc))
	require.NoError(t, err)
	defer cl.Close()
