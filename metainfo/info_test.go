...
func TestOffsetToIndex(t *testing.T) {
	t.Run("128 KiB", func(t *testing.T) {
		info, err := NewFromReader(io.LimitReader(cryptox.NewChaCha8(t.Name()), 128*bytesx.KiB), OptionPieceLength(bytesx.KiB))
		require.NoError(t, err)
		...
		require.Equal(t, int64(127), info.OffsetToIndex(128*bytesx.KiB))
	})

	t.Run("1 MiB", func(t *testing.T) {
		info, err := NewFromReader(io.LimitReader(cryptox.NewChaCha8(t.Name()), 4*bytesx.MiB))
		require.NoError(t, err)

		require.Equal(t, int64(0), info.OffsetToIndex(0))
		require.Equal(t, int64(0), info.OffsetToIndex(1))
		require.Equal(t, int64(0), info.OffsetToIndex(bytesx.MiB-1))
		require.Equal(t, int64(1), info.OffsetToIndex(bytesx.MiB))
		require.Equal(t, int64(1), info.OffsetToIndex(2*bytesx.MiB-1))
		require.Equal(t, int64(2), info.OffsetToIndex(2*bytesx.MiB))
		require.Equal(t, int64(3), info.OffsetToIndex(4*bytesx.MiB))
		require.Equal(t, int64(3), info.OffsetToIndex(4*bytesx.MiB))
		require.Equal(t, int64(3), info.OffsetToIndex(4*bytesx.MiB))
		require.Equal(t, int64(3), info.OffsetToIndex(4*bytesx.MiB))
	})
